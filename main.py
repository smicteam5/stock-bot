...
WATCH_INSTITUTIONS = ["국민연금", "삼성자산운용", "미래에셋자산운용",
                      "한국투자", "KB자산운용", "신한자산운용"]
...
async def fetch_investor_flow(session, investor: str, trade: str, top_n: int = 7) -> list:
    ...
    try:
        ...
        top = results[:top_n]
        for item_r in top:
            item_r["sector"] = await fetch_sector(session, item_r["isin"]) if item_r.get("isin") else ""
        return top
    except Exception as e:
...
def format_flow_lines(items: list, trade: str) -> list:
    ...
    for i, s in enumerate(items, 1):
        ...
        lines.append(
            f"  {emoji} {i}. *{s['name']}*{sector_str} "
            f"시총대비 {s['ratio']:+.3f}% ({s['net']:+,.0f}백만원{chg_str})"
        )
    return lines
...
async def fetch_krx_supply(session, market: str) -> pd.DataFrame:
    """KRX API로 투자자별 순매수 직접 수집 (pykrx 없이)"""
    ...
    if not rows:
        raise ValueError(f"KRX 데이터 없음 ({market})")

    records = []
    for row in rows:
        try:
            dt = datetime.strptime(row["TRD_DD"].replace("/", "").replace(".", ""), "%Y%m%d")
            records.append({
                "date":   dt,
                "지수":   float(row.get("CLSPRC_IDX", 0) or 0),
                "외국인": float(row.get("FRGN_NETBID_TRDVAL", 0) or 0) / 1e8,
                "기관":   float(row.get("ORG_NETBID_TRDVAL",  0) or 0) / 1e8,
                "연기금": float(row.get("PNSN_NETBID_TRDVAL", 0) or 0) / 1e8,
                "개인":   float(row.get("INDV_NETBID_TRDVAL", 0) or 0) / 1e8,
            })
        except Exception:
            continue
...
async def send_supply_cumulative(app: Application, chat_id: str = None):
    ...
    await app.bot.send_message(chat_id, "⏳ 수급 누적 데이터 수집 중... (30초~1분 소요)")
    async with aiohttp.ClientSession() as session:
        for market in ["KOSPI", "KOSDAQ"]:
            try:
                df  = await fetch_krx_supply(session, market)
...
def main():
    ...
    application.add_handler(CommandHandler("supply", cmd_supply))
    ...
    async def post_init(app: Application):
        scheduler.start()
        print(f"[{datetime.now(KST)}] ✅ 봇 시작 완료")
        print("📌 /supply 명령어로 수급 누적 조회 가능")
